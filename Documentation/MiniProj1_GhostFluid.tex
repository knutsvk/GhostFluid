...
\pgfplotsset{width=\columnwidth, height=0.6\columnwidth, compat=1.12}
...
velocity $u$, total energy $E$ and pressure $p$, they are given in
...
\end{equation}
%
where the vector of conserved quantities ${\bf U}$ and their fluxes ${\bf
F(U)}$ are given by
...
= \begin{pmatrix} \rho, & u, & p \end{pmatrix}^T$. The total energy is the sum
    of the kinetic and potential energy of the system, i.e.~ 
...
equation of state. The equation of state is of fundamental importance for two
reasons.  Firstly, it allows us to treat the materials in question from a
continuum point of view. It is thus the connection between the physics taking
place on a microscopic scale and the thermodynamics which we apply
macroscopically.  Secondly, it provides the missing fourth equation to our four
unknowns ($\rho$, $u$, $p$ and $E$), so that the system becomes mathematically
determined. For an ideal gas, the equation of state is 
...
available. The development of numerical methods for partial differential
equations first took off after the classical paper by Courant, Friedrichs and
Lewis \cite{courant1928partiellen}, but local Riemann problems
\cite{godunov1959difference}, conservative methods \cite{lax1960systems} and
finite volume formulations \cite{leveque2002finite} were necessary before the
first proper, three-dimensional simulations of the Euler equations could be
performed in the 1980s \cite{jameson1981numerical}.  The Monotonic
Upstream-centered Scheme for Conservation Laws (MUSCL) \cite{van1979towards}
was introduced by van Leer as the first higher-order Total Variation
Diminishing (TVD) scheme in 1979. Since then, several others have been
developed, notably Toro's Weighted Average Flux (WAF) \cite{toro1989weighted}
and Flux/Slope LImiter Centred (SLIC/FLIC) \cite{toro2000centred} schemes. 
...
Euler equations.  From a continuum fluid mechanics point of view, multimaterial
interactions are used to describe problems where there is a relatively sharp
interface between two materials. Examples of such cases are water/oil
separation (liquid-liquid interaction), penguins swimming through water
(solid-liquid interaction) and water splashing into the air as a result of a
surface disruption (liquid-gas interaction). In reality, all these interfaces
are continuous from one material to the other. Moreover, because the length
scale of the interface is far smaller than even the discrete control volumes
used for simulation, it is a good approximation to treat the interfaces as
sharp discontinuities.  It is the goal of this report to demonstrate some
techniques available for multimaterial simulations of the Euler equations in
one dimension. Different fluids are characterized by their material properties,
...
interfaces must be modelled accurately. In order to do so, Fedkiw et al.\
developed the Original Ghost Fluid Method (OGFM)
...
OGFM, the pressures and velocities at these ghost cells are set equal to those
of the other material, and the densities are calculated by enforcing constant
entropy across the interface.  This worked well for many cases, but the method
breaks down when strong shocks are present in the solution, because then the
entropy condition does not apply.  As a response, the Modified Ghost Fluid
Method (MGFM) was developed \cite{liu2003ghost, sambasivan2009ghost}. It is
based on the realization that Riemann problems do not in general require the
states on each side of the discontinuity to correspond to the same material.
The cells in the ghost fluid are therefore updated by solving a local Riemann
problem. A further development is the real Ghost Fluid Method (rGFM)
\cite{wang2006real}, in which a more accurate interfacial boundary condition is
applied. The implementation of the rGFM is outside the scope of this report. 
...
in Section \ref{sec:results}. In Section \ref{sec:further}, we propose
possibilities for further work, while Section \ref{sec:conclusion} concludes the
report.
...
type. In order to solve the Riemann problem, one must therefore be able to
compute all these states, and also determine which of them is the correct one
based on the speeds of the waves. 
...
It can be proven \cite{toro2013riemann} that the pressure in the star states
$p_{*}$ is given by the root of the nonlinear function
...
iterative scheme to Eq.~ \eqref{eq:pressureEquation}. Computation of the roots
of $f(p)$ is a problem well-suited for iterative schemes, but it is important
to ensure that the pressure stays positive. For this project, Newton-Raphson
iterations were implemented, taking the initial guess as
$\frac{1}{2}(p_L+p_R)$. After finding $p_*$, the velocity in the star region is
computed as
...
computational domain, substantial gains in terms of computational efficiency
can be achieved by employing approximate solvers instead. One such solver is
the HLLC solver \cite{toro1994restoration}, which has been implemented for this
project. 
...
of the $i$-th cell, i.e.\ $x_i = (i+1/2)\Delta x$.  Temporal discretization is
done with variable time step $\Delta t^n$, so that $t^n = \sum_{i=1}^n \Delta
t^i$. 

We write the discrete approximation of ${\bf U}(x_i, t^n)$ as ${\bf U}_i^n$,
and let it be the weighted average of ${\bf U}(x,t^n)$ in the cell with
boundaries at $x_{i-1/2}$ and $x_{i+1/2}$: 
...
amount of neighbouring points needed in the employed update scheme. 
...
implemented so-called TVD high resolution methods for this purpose. High
resolution methods are accurate to at least second order; they avoid spurious
oscillations in the solution, capture discontinuities and shock waves well and
satisfy a discrete form of the entropy condition. There are two main classes of
higher-order flux evaluation schemes; the upwind schemes are based on the
solution of local Riemann problems, while the centered schemes are extensions
of the First ORder CEntered (FORCE) flux: 
...
\begin{equation}
    ...
    \frac{1}{2} \frac{\Delta t^n}{\Delta x}({\bf F}_R - {\bf F}_L) \,.
\end{equation}
%
In general, upwind schemes offer higher accuracy than centered ones, but they
...
First, the data cell averages ${\bf U}_i^n$ are replaced locally by linear
functions so that at the left and right cell interfaces, the boundary
extrapolated values are ${\bf U}_i^L = {\bf U}_i^n-\frac{1}{2}\Delta_i$ and
${\bf U}_i^R = {\bf U}_i^n+\frac{1}{2}\Delta_i$.  Here, the slope vector
$\Delta_i$ has been introduced, which in our case is chosen such that the
schemes are TVD. For the MUSCL-Hancock scheme, we have implemented limited
slopes \cite{anderson1986comparison}, while SLIC employs slope limiters
\cite{toro2000centred}. 

After calculating the boundary extrapolated values for each cell, these are
advanced to time $t^n+\frac{1}{2} \Delta t^n$ to obtain

\begin{equation}
    \label{eq:evolution}
    {\bf \bar{U}}_i^{L,R} = {\bf U}_i^{L,R} - \frac{1}{2} \frac{\Delta
    t^n}{\Delta x} \left[ {\bf F}({\bf U}_i^R)-{\bf F}({\bf U}_i^L) \right] \,.
\end{equation}
...
problem across each cell interface, i.e.\ 
...
function $\phi(x)$ whose sign is a boolean indicator of the material present.
The interfaces between materials are thus located at the points where $\phi=0$.
...
letting $\phi$ be piecewise linear throughout the domain. 
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}[scale=3.0]
        ...
        \node[fill=blue!20!white] () at (0.5,0.0) {Material A};
        \node[fill=green!20!white] () at (1.5,0.0) {Material B};
    \end{tikzpicture}
    \caption{Level-set function for a single material interface.
...
sure that the zeros end up between cell centers, so that we avoid grid points
where the level set function is exactly zero. By doing so, each grid point has
well-defined material properties. In order to capture the evolution of the
system, the level-set function itself must be advanced in time. Since $\phi$ is
a consequence of the state of the fluid rather than a property, it is not
coupled to the other variables in the system.  Thus it is governed by the
advection equation 
...
interesting information contained in $\phi$ are these locations, however,
non-conservative methods can be safely implemented as long as $\phi$ remains
smooth and not too steep near zeros. In our implementation, we have used the
...
$|\phi_{i+1}^n-\phi_i^n|=\Delta x$. We also make sure that
...
consistent in order to capture the physical interactions between materials.
Presence of thermodynamical inconsistencies leads to spurious entropy
generation, which in turn results in unphysical evolution of the system.  In
...
The first successful attempt at implementing a ghost fluid method was performed
by Fedkiw et al.\ in 1998 \cite{fedkiw1999nonoscillatory}. In this approach,
which we refer to as OGFM, the entropy $S$ is held constant across the ghost
fluid interface, in order to give conditions for thermodynamic consistency. The
...
corresponding cells in the other material domain. It is the density which is
...
Riemann problem across the interface. It is unproblematic to solve a Riemann
problem with different material properties on each side of the initial
discontinuity. Given the two states immediately adjacent to the material
interface, ${\bf W}_L$ and ${\bf W}_R$, and the corresponding material
properties, we calculate the star states ${\bf W}_{*L}$ and ${\bf W}_{*R}$, and
set the ghost fluid cells equal to them. Thus every extrapolated value in the
ghost fluid corresponds to a physical state of the system, and we have
thermodynamical consistency. By looking at Figure \ref{fig:riemann}, it is easy
to convince oneself that the chosen star states are valid choices for the
boundary cells.  In order to compute the star states, the exact solver is
employed. 

\subsection{Final notes on implementation}
\label{subsec:final}

With all separate parts of the computational procedure explained, it is helpful
to explain how it all fits together. As such, Algorithm \ref{alg:complete}
exhibits pseudocode summarizing the complete procedure. Our code is written in
C++, but for easy input customization and result visualization we have built a
user interface using Qt, a cross-platform application framework. 
...
methods cope under a range of circumstances. Several methods have been
described, and several variations exist within each method.  We therefore
inform that unless otherwise specified, fluxes are computed using the
MUSCL-Hancock scheme; slope limiters are of minbee type; OGFM is implemented
with the isobaric fix.  The reference solutions, where available, are obtained
using the exact solver with 5000 points. Although the amount of grid points
varies throughout the results, only 50 points in the solutions are included in
all plots, so as to avoid the high-accuracy tests overshadowing the information
from the lower accuracy ones.
...
Results for test A are shown in Figure \ref{fig:testA}. Note that only
...
schemes, hence its alias ``the Sod test tube''. Figure \ref{fig:toro1density}
exhibits the results for density for both ghost fluid methods. In Figure
\ref{fig:toro1}, results for all variables with several different resolutions
are displayed.  All features are captured very well by both models. 

\pgfplotstableread{../Results/Toro1_OGFM_FORCE_N5000.out}{\exact}
\pgfplotstableread{../Results/Toro1_MGFM_MUSCL_minbee_N100.out}{\mgfm}
\pgfplotstableread{../Results/Toro1_OGFM_MUSCL_minbee_N100.out}{\ogfm}
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}
            ...
            table[x={x}, y={rho}] {\ogfm}; 
            \addlegendentry{OGFM};
            \addplot+[only marks, mark=o, color=blue, each nth point=2] 
            table[x={x}, y={rho}] {\mgfm}; 
            \addlegendentry{MGFM};
        \end{axis}
    \end{tikzpicture}
    \caption[caption]
    {
        Density for OGFM and MGFM  applied to Toro's test 1 with $N=100$.
    }
    \label{fig:toro1density}
\end{figure}

This test also lends itself well to display the improvement that can be
achieved in OGFM by applying the isobaric fix. From Figure
...
\pgfplotstableread{../Results/Toro1_OGFM_FORCE_N5000.out}{\exact}
\pgfplotstableread{../Results/Toro1_OGFMIF_MUSCL_minbee_N100.out}{\ogfmif}
\pgfplotstableread{../Results/Toro1_OGFM_MUSCL_minbee_N100.out}{\ogfm}
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}
            ...
            \addlegendentry{Exact};
            \addplot+[only marks, mark=triangle, color=red] 
            table[x={x}, y={e}] {\ogfmif}; 
            \addlegendentry{With i.f.};
            \addplot+[only marks, mark=o, color=blue] 
            table[x={x}, y={e}] {\ogfm}; 
            \addlegendentry{Without i.f.};
        \end{axis}
    \end{tikzpicture}
    \caption[caption]
    {
        Internal energy for OGFM applied to Toro's test 1 with and without
        isobaric fix. For both cases, $N=100$.
    }
...
\pgfplotstableread{../Results/Toro3_MGFM_MUSCL_minbee_N100.out}{\min}
\pgfplotstableread{../Results/Toro3_MGFM_MUSCL_superbee_N100.out}{\super}
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}
            ...
            \addplot+[only marks, mark=triangle, color=red] 
            table[x={x}, y={rho}] {\min}; 
            \addlegendentry{Minbee};
            \addplot+[only marks, mark=o, color=blue] 
            table[x={x}, y={rho}] {\super}; 
...
opposite directions compared to test 3. Figure \ref{fig:toro4} confirms the behaviour of our
solvers when applied to tests containing large shock waves. 

\subsubsection{Test 5}

As a last example, the fifth test case mimics what happens when the right-moving
shock wave from test 3 meets the left-moving shock wave from test 4. As seen
...
correspond to physically realistic scales, measured in SI units. 

\subsubsection{Test C}

\pgfplotstableread{../Results/MPC_OGFM_FORCE_N5000.out}{\exact}
\pgfplotstableread{../Results/MPC_OGFMIF_MUSCL_minbee_N100.out}{\OGFMone}
\pgfplotstableread{../Results/MPC_OGFMIF_MUSCL_minbee_N200.out}{\OGFMtwo}
\pgfplotstableread{../Results/MPC_OGFMIF_MUSCL_minbee_N400.out}{\OGFMfour}
\pgfplotstableread{../Results/MPC_OGFMIF_MUSCL_minbee_N2000.out}{\OGFMtausen}
\pgfplotstableread{../Results/MPC_MGFM_MUSCL_minbee_N100.out}{\MGFMone}
\pgfplotstableread{../Results/MPC_MGFM_MUSCL_minbee_N200.out}{\MGFMtwo}
\pgfplotstableread{../Results/MPC_MGFM_MUSCL_minbee_N400.out}{\MGFMfour}
\pgfplotstableread{../Results/MPC_MGFM_MUSCL_minbee_N2000.out}{\MGFMtausen}
...
\begin{figure*}[t]
    ...
    \begin{tikzpicture}
        ...
        [ 
            ...
            \addplot+[mark=none, color=black] table[x={x}, y={rhoEx}]{\exact};
            \addplot+[only marks, mark=triangle, color=red, each nth point=2]
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for Test C using OGFM (left) and MGFM (right).
...
towards the right and refracts at an interface with helium gas. The result of
the interfacial interaction is a narrow, reflected rarefaction wave and a weak,
transmitted shock wave. The ratio of specific heats $\gamma$ (and thus the
speed of sound $a$) is higher for helium than for air, so the transmitted shock
wave travels faster than the incident shock wave in air. Initial parameters
for the left (air postshock), middle (air preshock) and right (heliúm preshock)
parts of the domain are tabulated in Table \ref{tab:testC}, and the simulation
is run to a final time $t_{\rm end}=0.0012$. 
...
\subsubsection{Test D}

An interesting test case developed by Quirk and Karni \cite{quirk1996dynamics}
...
See Table \ref{tab:testD} for the initial conditions, and Figure
\ref{fig:testDinit} for a plot of the initial density distribution. Due to the
presence of a local Riemann problem in the left air region in addition to two
material interfaces, the resulting pattern becomes quite complex. The shock
wave refracts at both material interfaces, so the end result (with $t_{\rm
...
\pgfplotstableread{../Results/MPD_init.out}{\exact}
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}
            ...
            \addplot+[mark=none, color=black] table[x={x}, y={rho}]{\exact};
        \end{axis}
    \end{tikzpicture}
    \caption[caption]
    {
        Initial density distribution for test D. A slab of helium gas is
        located in the region $0.4 < x \leq 0.6$, while the rest of the domain
        is filled with hydrogen. 
    }
    \label{fig:testDinit}
\end{figure}

\pgfplotstableread{../Results/MPD_OGFMIF_MUSCL_minbee_N100.out}{\OGFMone}
\pgfplotstableread{../Results/MPD_OGFMIF_MUSCL_minbee_N200.out}{\OGFMtwo}
\pgfplotstableread{../Results/MPD_OGFMIF_MUSCL_minbee_N400.out}{\OGFMfour}
\pgfplotstableread{../Results/MPD_OGFMIF_MUSCL_minbee_N2000.out}{\OGFMtausen}
\pgfplotstableread{../Results/MPD_MGFM_MUSCL_minbee_N100.out}{\MGFMone}
\pgfplotstableread{../Results/MPD_MGFM_MUSCL_minbee_N200.out}{\MGFMtwo}
\pgfplotstableread{../Results/MPD_MGFM_MUSCL_minbee_N400.out}{\MGFMfour}
\pgfplotstableread{../Results/MPD_MGFM_MUSCL_minbee_N2000.out}{\MGFMtausen}
...
\begin{figure*}[t]
    ...
    \begin{tikzpicture}
        ...
        [ 
            ...
            \nextgroupplot[ylabel=$\rho$]
            \addplot+[only marks, mark=*, mark size=0.2, color=purple, 
            each nth point=2]
            table[x={x}, y={rho}]{\OGFMtausen};
            \addplot+[only marks, mark=triangle, color=red, each nth point=2]
            table[x={x}, y={rho}]{\OGFMone};
            \addplot+[only marks, mark=o, color=blue, each nth point=4]
            table[x={x}, y={rho}]{\OGFMtwo};
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for Test D using OGFM (left) and MGFM (right).
    \textcolor{red}{$\triangle$}$N=100$
    \textcolor{blue}{$\circ$}$N=200$
    \textcolor{teal}{$\times$}$N=400$ 
    \textcolor{purple}{$\cdot$}$N=2000$ }
\label{fig:testD}
\end{figure*}
...
be found e.g.\ in a paper by Wang et al.\ from
...
gases, i.e.\ which have $p_{\infty}=0$ in  Eq.\ \eqref{eq:stiffEoS}. An
example of a fluid which is approximated quite well by the stiffened gas
equation of state is water. As a final validation test case, we therefore
verify the applicability of our model to water. 

\subsubsection{Test E}

This test case is a standard shock-tube problem, but where the high-density
region is filled with water, following Chinnaya et al.\
...
become difficult to distinguish due to this huge difference. Test E is run to a
final time $t_{\rm end}=237.44\cdot 10^{-6}$. 
...
The results for test E are shown in Figure \ref{fig:testE}. We did not expect
OGFM to deal well with the large differences in equation of state, but it
...
\pgfplotstableread{../Results/MPE_OGFM_FORCE_N5000.out}{\exact}
\pgfplotstableread{../Results/MPE_OGFMIF_MUSCL_minbee_N100.out}{\OGFMone}
\pgfplotstableread{../Results/MPE_OGFMIF_MUSCL_minbee_N200.out}{\OGFMtwo}
\pgfplotstableread{../Results/MPE_OGFMIF_MUSCL_minbee_N400.out}{\OGFMfour}
\pgfplotstableread{../Results/MPE_MGFM_MUSCL_minbee_N100.out}{\MGFMone}
\pgfplotstableread{../Results/MPE_MGFM_MUSCL_minbee_N200.out}{\MGFMtwo}
\pgfplotstableread{../Results/MPE_MGFM_MUSCL_minbee_N400.out}{\MGFMfour}
...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for test E using OGFM (left) and MGFM (right). 
    \textcolor{red}{$\triangle$}$N=100$
    \textcolor{blue}{$\circ$}$N=200$
    \textcolor{teal}{$\times$}$N=400$ }
\label{fig:testE}
...
The results from test D show that our level set function can track two
material interfaces simultaneously without a problem. We wanted to ensure that
it could track an arbitrary amount of interfaces, however, and therefore
created an extra test where the amount of interfaces has been doubled compared
to the previous maximum. 

\subsubsection{Test F}

Table \ref{tab:testF} shows the initial conditions for test F, which is run to
a final time $t_{\rm end}=0.1$. The leftmost, center and rightmost regions
contain hydrogen gas, and two slabs of helium are placed between these. Due to
the initial discontinuities, shock waves travel toward the interior of both
helium slabs, where they collide and interact.  This is not necessarily a very
realistic case physically, but it does test the ability of the scheme to
generate a zig-zagging level set function and track it correctly. 
...
\begin{table}[htb]
    ...
    \begin{tabular}{ccccc}
        ...
        \hline
        $[0,0.2]$ & 1 & 0 & 1 & 1.4 \\
        $(0.2,0.4]$ & 0.125 & 0 & 0.1 & 1.67 \\
        $(0.4,0.6]$ & 1 & 0 & 1 & 1.4 \\
        $(0.6,0.8]$ & 0.125 & 0 & 0.1 & 1.67 \\
        $(0.8,1]$ & 1 & 0 & 1 & 1.4 \\
        \hline
    \end{tabular}
    \caption{Initial conditions for test F.}
    \label{tab:testF}
\end{table}

Results generated by applying MGFM to test F are shown in Figure
\ref{fig:testF}. Since no exact solution is possible, and no comparison is
provided in the literature, we ran the code with a high resolution mesh ($N=2000$).
The resulting patterns are complex and difficult to interpret from just a
snapshot, but animating the results in our graphical user interface show that
the system does indeed behave in an intuitive manner. 

\pgfplotstableread{../Results/MPF_MGFM_MUSCL_minbee_N2000.out}{\mgfm}
...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}
        ...
        [ 
            ...
            \nextgroupplot[ylabel=$\rho$]
            \addplot+[only marks, mark=*, mark size=0.2, color=blue]
            table[x={x}, y={rho}]{\mgfm};
        ...
        \end{groupplot}
    \end{tikzpicture}
    \caption{Results for test F using MGFM.}
\label{fig:testF}
\end{figure*}
...
For future development of the code, we suggest extensions to two (and three)
spatial dimensions, so as to facilitate simulations of more interesting
physical problems that are relatable to real-world applications. In order to do
so, we would advise implementing dimensional operator splitting. For these types of
problems, it would also be interesting to investigate arbitrary domain shapes
and mesh refinement techniques. Since the computational expense grows quickly
with the amount of dimensions, it would at this point be feasible to put some
effort into speeding up the runtime. Luckily, the most expensive job
is the calculation of fluxes, which can be done independently for each point in the
domain. Thus, parallelization techniques should be implemented for high gains
in terms of computing time. 

\section{Conclusions}
\label{sec:conclusion}

We have successfully implemented a numerical PDE solver for the compressible
Euler equations, which allows for multimaterial continuum simulations in one
spatial dimension. Although there is nothing novel about the methods and
resulting code, the project report serves as a literature review which covers
an introduction to the Euler equations and their numerical solution, in
addition to multimaterial problems for which level set and ghost fluids methods
are necessary. In addition to describing the theory, we present results for
numerous test cases which illustrate that our code is correct, robust and
in agreement with what is described in the literature. 

\section*{Acknowledgements}
\label{sec:acknowledgements}
Gratitude is directed to project advisor Stephen Millmore for sharing his
insight in the field and offering good advice. 
...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for Toro's test 1 using OGFM (left) and MGFM (right). 
    \textcolor{red}{$\triangle$}$N=100$
    \textcolor{blue}{$\circ$}$N=200$
    \textcolor{teal}{$\times$}$N=400$ }
\label{fig:toro1}
...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for Toro's test 2 using OGFM (left) and MGFM (right). 
    \textcolor{red}{$\triangle$}$N=100$
    \textcolor{blue}{$\circ$}$N=200$
    \textcolor{teal}{$\times$}$N=400$ }
\label{fig:toro2}
...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}
        ...
        [ 
            ...
            \nextgroupplot[ylabel=$\rho$, ymax=9.5]
            \addplot+[mark=none, color=black]
            table[x={x}, y={rhoEx}]{\exact};
            \addplot+[only marks, mark=triangle, color=red, each nth point=2]
            table[x={x}, y={rho}]{\OGFMone};
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for Toro's test 3 using OGFM (left) and MGFM (right). 
    \textcolor{red}{$\triangle$}$N=100$
    \textcolor{blue}{$\circ$}$N=200$
    \textcolor{teal}{$\times$}$N=400$ }
\label{fig:toro3}
...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}
        ...
        [ 
            ...
            \nextgroupplot[ylabel=$\rho$, ymax=8.5]
            \addplot+[mark=none, color=black]
            table[x={x}, y={rhoEx}]{\exact};
            \addplot+[only marks, mark=triangle, color=red, each nth point=2]
            table[x={x}, y={rho}]{\OGFMone};
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for Toro's test 4 using OGFM (left) and MGFM (right). 
    \textcolor{red}{$\triangle$}$N=100$
    \textcolor{blue}{$\circ$}$N=200$
    \textcolor{teal}{$\times$}$N=400$ }
\label{fig:toro4}
...
\begin{figure*}[htb]
    ...
    \begin{tikzpicture}
        ...
        \end{groupplot}
    \end{tikzpicture}
\caption{Results for Toro's test 5 using OGFM (left) and MGFM (right). 
    \textcolor{red}{$\triangle$}$N=100$
    \textcolor{blue}{$\circ$}$N=200$
    \textcolor{teal}{$\times$}$N=400$ }
\label{fig:toro5}
