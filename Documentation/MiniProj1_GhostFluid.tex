...
specified:
\begin{itemize}
    \item fluxes are computed using the MUSCL-Hancock scheme,
    \item slope limiters are of minbee type, 
    \item OGFM is implemented with the isobaric fix.
\end{itemize}
...
separates two different fluids; namely air and helium ($\gamma=1.67$). As such, only the GFM
solvers have been utilized. 
...
\begin{figure}[htb]
    ...
    \begin{tikzpicture}
        \begin{axis}
            [xlabel=$x$, xmin=0.6, xmax=0.9, ylabel=$\rho$, ylabel
            style={rotate=-90}, legend style={font=\footnotesize}]
            \addplot+[mark=none, color=black] 
            table[x={x}, y={rhoEx}]{\exact};
            \addlegendentry{Exact};
