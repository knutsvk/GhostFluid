...
has been implemented following Toro's comprehensive overview of Riemann solvers
and their uses \cite{toro2013riemann}, but extended to take into consideration
that different (stiffened) ideal gases may be present on each side of the
discontinuity. 
...
\begin{equation}
    u_{*} = \frac{1}{2} (u_L+u_R)-\frac{1}{2}\left[ f_L(p_*)-f_R(p_*) \right] \,.
    \label{eq:ustar}
\end{equation}
...
require the solution of local Riemann problems at all points in the
