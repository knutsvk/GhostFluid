...
When the Euler equations are given as in Eq.\ \eqref{eq:euler}, they are said
to be a system of conservation equations in differential form. In integral
form, on a spatial domain $[x_L, x_R]$, the same system of equations can be
written
...
\begin{align}
    ...
    -\int_{t_1}^{t_2} {\bf F}({\bf U}(x_R,t)) {\rm d}x \\
    &+ \int_{t_1}^{t_2} {\bf F}({\bf U}(x_L,t)) {\rm d}x \,.
