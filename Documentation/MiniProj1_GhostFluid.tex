...
equation of state. This equation is of fundamental importance for two reasons.
Firstly, it allows us to treat the materials in question from a continuum point
of view. It is thus the connection between the physics taking place on a
microscopic scale and the thermodynamics which we apply to the macroscopic one.
Secondly, it provides the missing fourth equation to our four unknowns ($\rho$,
$u$, $p$ and $E$), so that the system becomes mathematically consistent. For an ideal gas, the equation of state is 
...
provided in the literature, we ran the code with a high resolution ($N=2000$).
